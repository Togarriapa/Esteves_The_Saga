...
public class Puff extends Obstacle {
    private Picture picture;
    private String filepath = Program.prefix + "PillowGreenAsset.png";
    ...
    public Puff() {
        ...
        position = new Position(Grid.gridMaxCol-(width/Grid.gridSize),
                Grid.gridMaxCol-2,
                Grid.gridMaxRow-(height/Grid.gridSize) - getBottomToFloor(),
                Grid.gridMaxRow - getBottomToFloor());

        rectangle = new Rectangle((Grid.gridMaxCol*Grid.gridSize)-width,
                ((Grid.gridMaxRow*Grid.gridSize)-height )-getBottomToFloorPixel(),
                width,height);

        picture = new Picture(position.getMinCol()*10, (position.getMinRow()*10),filepath);
